...
double Norm(const std::vector<double> &vector) {
    double norm = 0.0;
#if defined(SCHEDULE_DYNAMIC)
#pragma omp parallel for num_threads(threads) schedule(dynamic) reduction(+:norm)
#elif defined(SCHEDULE_GUIDED)
#pragma omp parallel for num_threads(threads) schedule(guided) reduction(+:norm)
#elif defined(SCHEDULE_AUTO)
#pragma omp parallel for num_threads(threads) schedule(auto) reduction(+:norm)
#else
#pragma omp parallel for num_threads(threads) schedule(static) reduction(+:norm)
#endif
    for (int i = 0; i < N; ++i)
...
int main(int argc, char **argv) {
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <matrix_size> <tau> <num_threads>" << std::endl;
        return 1;
    }
